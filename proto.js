    ...
    fetch('./blogs/posts.json')
        ...
        .then(posts => {
            ...
            posts.forEach(post => {
                const postElement = document.createElement('article');
                postElement.innerHTML = `
                    <h3><a href="blogpost.html" class="blog-post-link" data-post="${post.file}">${post.title}</a></h3>
                    <p>${post.excerpt}</p>
                `;
                blogContainer.appendChild(postElement);
            });
                ...
                link.addEventListener('click', (e) => {
                    ...
                    fetch(`./blogs/${postFile}`)
                        ...
                        .then(markdown => {
                            ...
                            // Store the content in sessionStorage
                            sessionStorage.setItem('blogPostContent', html);
                            sessionStorage.setItem('blogPostTitle', postTitle);
                            
                            // Let the browser navigate to blogpost.html
                        })
                        .catch(error => {
                            console.error('Error loading blog post:', error);
                            alert('Failed to load blog post. Please try again later.');
